<%@ page language="java" contentType="text/html; charset=EUC-KR"
    pageEncoding="utf-8"%>
<!DOCTYPE html>
<html>
<head>
	<title>MY HOB!</title>
	<meta charset="utf-8" />
  	<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no" />
	<link rel="stylesheet" href="../assets/css/main.css"/>
	<link rel="stylesheet" href="../assets/css/Q&A.css">
</head>
<body class="homepage is-preload">
		<div id="page-wrapper">

			<!-- Header -->
				<section id="header">
					<div class="container">

						<!-- Logo -->
							<h1 id="logo"><a href="index.jsp">MY HOB!</a></h1>
							<p>A responsive HTML5 site template. Manufactured by HTML5 UP.</p>

							<nav id="nav">
								<ul>
									<li><a class="icon solid fa-home" href="../index.jsp"><span>Introduction</span></a></li>
									<li>
										<a href="#" class="icon fa-chart-bar"><span>HOBBY</span></a>
										<ul>
											<li><a href="#">Lorem ipsum dolor</a></li>
											<li><a href="#">Magna phasellus</a></li>
											<li><a href="#">Etiam dolore nisl</a></li>
											<li>
												<a href="#">Phasellus consequat</a>
												<ul>
													<li><a href="#">Magna phasellus</a></li>
													<li><a href="#">Etiam dolore nisl</a></li>
													<li><a href="#">Phasellus consequat</a></li>
												</ul>
											</li>
											<li><a href="#">Veroeros feugiat</a></li>
										</ul>
									</li>
									<li><a class="icon solid fa-cog" href="left-sidebar.jsp"><span>Left Sidebar</span></a></li>
									<li><a class="icon solid fa-retweet" href="../index.jsp">
									<span>Service Center</span></a>
										<ul>
											<li><a href="/ServiceCenter/notice.jsp">공지사항</a></li>
											<li><a href="/ServiceCenter/FAQ.jsp">FAQ</a></li>
											<li><a href="/ServiceCenter/Q&A.jsp">Q&A</a></li>
										</ul>
									</li>
									<li><a class="icon solid fa-sitemap" href="no-sidebar.jsp"><span>No Sidebar</span></a></li>
								</ul>
							</nav>
					</div>
				</section>
 		<div id="my-Sidebar">
        	<h2>고객센터</h2>
        		<ul>
         		  	<li><a href="../ServiceCenter/index.jsp" >공지사항</a></li>
        		  	<li><a href="../ServiceCenter/FAQ.jsp" >FAQ</a></li>
         	  	 	<li><a href="../ServiceCenter/Q&A.jsp" >Q&A</a></li>
        		</ul>
      	</div>
      	<section id="main"> 
			<div class="container">
		 		<div id="content">
                       <table>
                           <thead>
                                <tr>
                                   <th>No</th>
                                    <th>제목</th>
                                     <th>작성자</th>
                                     <th>작성일</th>
                                     <th>조회수</th>
                                  </tr>
                            </thead>
                       <tbody>
                             <tr>
                                <td>5</td>
                                <td>프로필 설정 때 꼭 제 얼굴을 넣어야 하나요?</td>
                                <td>김민설</td>
                                <td>2020/09/02</td>
                                <td>923</td>
                             </tr>
                             <tr>
                                <td>4</td>
                                <td>클래스 수강을 잘못 신청했는데 어디서 취소해야 하나요?</td>
                                <td>김정하</td>
                                <td>2020/09/02</td>
                                <td>425</td>
                             </tr>
                             <tr>
                                <td>3</td>
                                <td>취미 추천을 받을 수 있는 설문조사는 어디서 해야하나요?</td>
                                <td>김은비</td>
                                <td>2020/09/02</td>
                                <td>1221</td>
                            </tr>
                            <tr>
                                <td>2</td>
                                <td>마라톤 참가인원을 구하고 싶은데 어느 게시판을 이용해야 하나요?</td>
                                <td>송지언</td>
                                <td>2020/09/01</td>
                                <td>326</td>
                          </tr>
                           <tr>
                                <td>1</td>
                                <td>활동로그을 어디서 확인할 수 있나요?</td>
                                <td>삼김송</td>
                                <td>2020/09/01</td>
                                <td>326</td>
                          </tr>
                         </tbody>
                     </table>
                        </div>
                          </div>
                           </section>
		<!-- Scripts -->
			<script src="../assets/js/jquery.min.js"></script>
			<script src="../assets/js/jquery.dropotron.min.js"></script>
			<script src="../assets/js/browser.min.js"></script>
			<script src="../assets/js/breakpoints.min.js"></script>
			<script src="../assets/js/util.js"></script>
			<script src="../assets/js/main.js"></script>
	</body>
</html>